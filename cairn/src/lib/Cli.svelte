<script>
  ...
  // here: the list, the flags and the defaults all come from `cairn --help`, which clap
...
<div class="where">
  ...
  {:else if ref}
    ...
    <div class="asline">
      <code>cargo build --release -p cairn-cli</code>
      <button class="ghost tiny" onclick={() => copy("cargo build --release -p cairn-cli")}>
